...
__author__ = 'wesc+api@google.com (Wesley Chun), cmiller1989@gmail.com (Calvin Miller)'
...
# - - - Conference objects - - - - - - - - - - - - - - - - -
    ...
    def _createSessionObject(self, request):
        ...
        data = {field.name: getattr(request, field.name) for field in request.all_fields()}
        cwf_key = data['confwebsafeKey']
        del data['confwebsafeKey']

        # Check that the user is the organizer of the Conference
        user = endpoints.get_current_user()
        if not user:
            raise endpoints.UnauthorizedException('Authorization required')
        user_id = getUserId(user)
        ...
        # convert dates from strings to Date objects; set time
        if data['date']:
    ...
    def getConference(self, request):
        ...
        if not conf:
            raise endpoints.NotFoundException(
                'No conference found with key: %s' % request.websafeConfKeyrenceKey)
    ...
    def getHalfHourSessionsByLarry(self, request):
        ...
        q = q.filter(Session.speaker == 'Larry Scott')
        
        q = q.filter(Session.duration == 30)
    ...
    def getLateSessions(self, request):
        ...
        #Grab all sessions after 4 o clock
        q = q.filter(Session.startTime > time(16,0,0))
...
# - - - Registration - - - - - - - - - - - - - - - - - - - -
    ...
    @endpoints.method(WishListForm, BooleanMessage,
            path='addhToWishlist/test',
            http_method='POST', name='addSessionToWishlist')
    def addSessionToWishlist(self, request):
        """Register or unregister user for selected session in Wishlist."""
    ...
    def deleteSessionInWishlist(self, request):
        """Delete a current Session inside of a users wishlist."""
...
# - - - Announcements - - - - - - - - - - - - - - - - - - - -
    ...
    def _cacheFeaturedSpeakersAnnouncement(confwebsafeKey, featuredSpeaker):
        ...
        #Check if there is more than one session by this speaker.
        if sessions.count > 1:
            announcement = '%s %s' % (
                'Our featured speaker '
                'has the following sessions available as well',
                ', '.join(sess.name for sess in sessions))
            memcache.set(MEMCACHE_FEATURED_SPEAKERS_KEY, announcement)
        else:
            # If there are no sold out conferences,
            # delete the memcache announcements entry
            announcement = ""
    ...
    @endpoints.method(message_types.VoidMessage, StringMessage,
            path='conference/announcementFeaturedSpeakers/get',
            http_method='GET', name='getFeaturedSpeaker')
    def getFeaturedSpeaker(self, request):
        """Returns the featuredSpeaker Announcement from memcache."""
        announcement = memcache.get(MEMCACHE_FEATURED_SPEAKERS_KEY)
        if not announcement:
            announcement = ""
        return StringMessage(data=announcement)
